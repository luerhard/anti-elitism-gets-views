...
model_types <- list(
  views_elite = formula(
    video_views ~ channel + log_n_sents + released_year + is_short + elite
  ),
  views_pplcentr = formula(
    video_views ~ channel + log_n_sents + released_year + is_short + pplcentr
  ),
  likes_elite = formula(
    video_likes ~ channel + log_n_sents + released_year + is_short + elite
  ),
  likes_pplcentr = formula(
    video_likes ~ channel + log_n_sents + released_year + is_short + pplcentr
  )
)
...
for (model_type in names(model_types)) {
  ...
  for (party in parties) {
    ...
    if (!dir.exists(fparty)) {
      dir.create(fparty, recursive = T)
    }
